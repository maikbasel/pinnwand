...
// Base UI's mergeProps object-literal parameter rejects `data-*` keys via
// excess-property checks. Passing the defaults through this identity helper
// makes them a non-fresh value, which preserves the exact type while letting
// the data-attributes flow onto the rendered element.
const withDefaults = <T,>(defaults: T): T => defaults;
...
function SidebarProvider({
  ...
  const setOpen = useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      ...
      // Note: the upstream shadcn primitive persists collapse state to a cookie
      // here. This project forbids direct document.cookie writes (security.md),
      // so collapse state stays in-memory. The rail uses collapsible="none", so
      // there is no cross-reload state to persist regardless.
    },
    [setOpenProp, open]
  );
  ...
  // Expose a data-state="expanded|collapsed" for Tailwind styling.
...
function SidebarMenuSkeleton({
  ...
  // Random width between 50% and 90% so a loading list reads as varied rows.
  const width = useMemo(
    () =>
