...
| Consent screen shows and you approve, but the connector fails right after with `mcp_token_exchange_failed` | MCP clients register (DCR) without `token_endpoint_auth_method`; GoTrue ≤2.189 defaults them to a *confidential* client + `client_secret`, but the MCP SDK uses PKCE and sends no secret at token exchange | Kong injects `token_endpoint_auth_method=none` on `/auth/v1/oauth/clients/register` so DCR registers a *public* client (see `docker/kong.yml`). Verify with `curl -sX POST .../auth/v1/oauth/clients/register -H 'Content-Type: application/json' -d '{"client_name":"x","redirect_uris":["https://claude.ai/api/mcp/auth_callback"]}'` → `"client_type":"public"` |
| Same `mcp_token_exchange_failed` persists after the Kong fix, and the failing URL shows the *same* `server=<uuid>` as before | the client cached its earlier *confidential* registration; reconnecting reuses it | fully **remove** the connector and re-add it as new so it runs a fresh DCR |
| `mcp_token_exchange_failed` even with a fresh public client; GoTrue `auth` log shows `HS256 is not supported for ID token signing` / `500: Error generating ID token` on `POST /oauth/token` | clients request the `openid` scope, so GoTrue must mint an OIDC **ID token**, which cannot be signed with the symmetric HS256 secret | configure asymmetric JWT signing keys (`JWT_KEYS`/`JWT_JWKS` → `GOTRUE_JWT_KEYS`, `PGRST_JWT_SECRET`, `API_JWT_JWKS`); see `docs/deployment.md` |
