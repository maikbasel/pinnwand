...
First run: `pnpm install`, then `pnpm dev:up`, then `pnpm dev`. The default `apps/web/.env` already carries the public demo anon key + kong URL (Vite reads it from the app root; `apps/web/.env.example` is the template). Sign in by typing a seeded address (`alice@dev.local` owner, `bob@dev.local` member on "Team-Board"; `charlie@dev.local` owns "Charlies Board") on `/sign-in` and reading the magic link / OTP from Mailpit (`http://localhost:8025`) — no passwords, signup is disabled.
...
pnpm-workspace + Turborepo monorepo. The web app lives in `apps/web`; the shared contract surface (generated DB types + fixed column/priority constants) lives in `packages/contracts` and is imported as `@pinnwand/contracts`. `apps/mcp` is a remote MCP connector server (Express + `@modelcontextprotocol/sdk`) that verifies the caller's GoTrue JWT and acts as that user via a per-request anon-key Supabase client, so RLS is the only authorization boundary — it never holds the service role key. `supabase/` and `docker/` stay at the repo root, shared by both apps.
...
apps/mcp/                ← Remote MCP connector server (JWT-verified, RLS-scoped)
...
- `.env` files (`apps/web/.env` for the app, root `.env` for compose/prod) — reference only; never commit
- `dist/`, `node_modules/`
