...
pnpm db:types                     # Regenerate src/shared/types/database.ts from the dev db (never hand-edit)
```
...
- shadcn/ui primitives added via `pnpm dlx shadcn@latest add <name>`; theme "Modern Minimal" tokens in `src/app/index.css` (`:root` / `.dark`), font Inter.
